...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>JoyPubSubTopic</name>"
                                "<dataType>sensor_msgs::msg::Joy</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        sensor_msgs_msg_Joy topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = sensor_msgs_msg_Joy_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        sensor_msgs_msg_Joy_serialize_topic(&mb, &topic);
