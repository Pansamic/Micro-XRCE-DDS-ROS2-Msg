...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>JoyFeedbackPubSubTopic</name>"
                                "<dataType>sensor_msgs::msg::JoyFeedback</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        sensor_msgs_msg_JoyFeedback topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = sensor_msgs_msg_JoyFeedback_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        sensor_msgs_msg_JoyFeedback_serialize_topic(&mb, &topic);
