...
int main(void)
{
    ...
    while(true)
    {
        sensor_msgs_msg_PointCloud2 topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = sensor_msgs_msg_PointCloud2_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        sensor_msgs_msg_PointCloud2_serialize_topic(&mb, &topic);
