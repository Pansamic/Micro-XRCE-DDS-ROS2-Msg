...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>HeaderPubSubTopic</name>"
                                "<dataType>std_msgs::msg::Header</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        std_msgs_msg_Header topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = std_msgs_msg_Header_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        std_msgs_msg_Header_serialize_topic(&mb, &topic);
