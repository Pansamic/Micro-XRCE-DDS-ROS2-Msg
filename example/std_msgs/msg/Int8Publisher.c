...
int main(void)
{
    ...
    while(true)
    {
        std_msgs_msg_Int8 topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = std_msgs_msg_Int8_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        std_msgs_msg_Int8_serialize_topic(&mb, &topic);
