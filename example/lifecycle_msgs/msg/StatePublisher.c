...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>StatePubSubTopic</name>"
                                "<dataType>lifecycle_msgs::msg::State</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        lifecycle_msgs_msg_State topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = lifecycle_msgs_msg_State_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        lifecycle_msgs_msg_State_serialize_topic(&mb, &topic);
