...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>TFMessagePubSubTopic</name>"
                                "<dataType>tf2_msgs::msg::TFMessage</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        tf2_msgs_msg_TFMessage topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = tf2_msgs_msg_TFMessage_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        tf2_msgs_msg_TFMessage_serialize_topic(&mb, &topic);
