...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>TF2ErrorPubSubTopic</name>"
                                "<dataType>tf2_msgs::msg::TF2Error</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        tf2_msgs_msg_TF2Error topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = tf2_msgs_msg_TF2Error_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        tf2_msgs_msg_TF2Error_serialize_topic(&mb, &topic);
