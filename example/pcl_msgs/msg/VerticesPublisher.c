...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>VerticesPubSubTopic</name>"
                                "<dataType>pcl_msgs::msg::Vertices</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        pcl_msgs_msg_Vertices topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = pcl_msgs_msg_Vertices_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        pcl_msgs_msg_Vertices_serialize_topic(&mb, &topic);
