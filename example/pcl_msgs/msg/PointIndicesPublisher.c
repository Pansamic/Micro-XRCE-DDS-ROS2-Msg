...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>PointIndicesPubSubTopic</name>"
                                "<dataType>pcl_msgs::msg::PointIndices</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        pcl_msgs_msg_PointIndices topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = pcl_msgs_msg_PointIndices_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        pcl_msgs_msg_PointIndices_serialize_topic(&mb, &topic);
