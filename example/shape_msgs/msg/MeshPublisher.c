...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>MeshPubSubTopic</name>"
                                "<dataType>shape_msgs::msg::Mesh</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        shape_msgs_msg_Mesh topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = shape_msgs_msg_Mesh_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        shape_msgs_msg_Mesh_serialize_topic(&mb, &topic);
