...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>GridCellsPubSubTopic</name>"
                                "<dataType>nav_msgs::msg::GridCells</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        nav_msgs_msg_GridCells topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = nav_msgs_msg_GridCells_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        nav_msgs_msg_GridCells_serialize_topic(&mb, &topic);
