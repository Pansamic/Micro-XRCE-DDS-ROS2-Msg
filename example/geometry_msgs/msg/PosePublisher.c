...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>PosePubSubTopic</name>"
                                "<dataType>geometry_msgs::msg::Pose</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        geometry_msgs_msg_Pose topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = geometry_msgs_msg_Pose_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        geometry_msgs_msg_Pose_serialize_topic(&mb, &topic);
