...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>WrenchPubSubTopic</name>"
                                "<dataType>geometry_msgs::msg::Wrench</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        geometry_msgs_msg_Wrench topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = geometry_msgs_msg_Wrench_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        geometry_msgs_msg_Wrench_serialize_topic(&mb, &topic);
