...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>ClockPubSubTopic</name>"
                                "<dataType>rosgraph_msgs::msg::Clock</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        rosgraph_msgs_msg_Clock topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = rosgraph_msgs_msg_Clock_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        rosgraph_msgs_msg_Clock_serialize_topic(&mb, &topic);
