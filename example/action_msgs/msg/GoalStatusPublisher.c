...
const char* topic_xml = "<dds>"
                            "<topic>"
                                "<name>GoalStatusPubSubTopic</name>"
                                "<dataType>action_msgs::msg::GoalStatus</dataType>"
                            "</topic>"
                        "</dds>";
...
int main(void)
{
    ...
    while(true)
    {
        action_msgs_msg_GoalStatus topic = {0};

        ucdrBuffer mb;
        uint32_t topic_size = action_msgs_msg_GoalStatus_size_of_topic(&topic, 0);
        uxr_prepare_output_stream(&session, reliable_out, datawriter_id, &mb, topic_size);
        action_msgs_msg_GoalStatus_serialize_topic(&mb, &topic);
